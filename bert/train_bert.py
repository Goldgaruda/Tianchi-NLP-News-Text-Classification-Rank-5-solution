...
#15是若不足15个字符，则用空格补齐；若超过了15个字符，原样输出。
...
fold_num = 20
data_file = '../data/pldata.csv'
import pandas as pd

def all_data2fold(fold_num, num=20000):
    fold_data = []
...
class WordBertEncoder(nn.Module):
    def __init__(self):
        ...
        #print(self.bert.embeddings.position_embeddings.weight)
        #print(self.bert.embeddings.position_embeddings.weight.shape)
    ...
    def _eval(self, epoch, test=False):
        ...
        with torch.no_grad():
            ...
            if test:
                ...
                if epoch >= 3:
                    logging.info(
                    'saving epoch {} middle csv file'.format(epoch))
                    ...
                    df = pd.DataFrame({'label': y_pred})
                    df.to_csv(save_test + str(epoch), index=False, sep=',')

                    #保存模型
                    torch.save(self.model.state_dict(), save_model + str(epoch))
